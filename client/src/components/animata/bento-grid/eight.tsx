...
export function FeatureTwo({
  ...
  return (
    <BentoCard className={cn("relative flex flex-col overflow-visible", className)}>
      <strong className={cn("text-2xl font-semibold", textClassName)}>
        <Counter targetValue={targetValue} format={(v) => `${Math.ceil(v)}${suffix}${label}`} className={textClassName} />
      </strong>
      {items && (
        <div className="ml-4 mt-auto">
          <AvatarList size="sm" className="py-0" items={items} />
        </div>
      )}
    </BentoCard>
  );
